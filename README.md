...
| 你要完成的任务 | 使用工具 | 工具负责的处理 |
| --- | --- | --- |
| 把检测报告、合同、表格或扫描 PDF 变成可编辑内容 | `xfyun_ocr` | PDF 逐页渲染、图片压缩、Markdown/HTML 表格、版面 JSON |
| 在原图上检查段落、标题、表格、公式、印章等区域 | `xfyun_ocr` | 按版面类型绘框并返回标注 PNG |
| 把文章、客服话术或课程内容生成自然语音 | `xfyun_tts` | 超过 64 KiB 自动拆分，合成 MP3/PCM 并安全写入文件 |
| 为直播、会议进行中或热线生成实时字幕 | `xfyun_rtasr` | 按实时节奏发送 PCM/Opus/Speex，返回最终文本和事件 |
| 整理会议、访谈、客服录音或多小时音频 | `xfyun_ifasr_submit` + `xfyun_ifasr_result` | 自动识别声道、分离发音人、生成时间轴/SRT/VTT，超限自动切片并支持中断续跑 |
| 转写前检查或转换声道、采样率、码率 | `xfyun_media` | 查询音频属性，完成单/双声道与采样率、码率转换 |
...
下面的快速注册命令使用 `@latest`，适合希望自动获得新版本的个人用户。团队或生产环境可改为固定版本（当前为 `@0.8.0`），避免未经验证的自动升级。
