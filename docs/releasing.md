...
5. 保持 Go、七个 npm 包、optionalDependencies、连接器及配置示例的版本一致，当前 npm 版本为 `0.4.1`；WorkBuddy 连接器按单独审核版本管理。
...
npm 版本不能重复发布。部分成功时，先用 `npm view <包名>@0.4.1` 核实已发布的包，只继续尚未发布的包；不要直接重新运行完整发布流程。若发布内容错误，需要提升所有相关版本后重新发布。

也可以上传仓库和 `v0.4.1` tag 后使用 GitHub Actions：设置 `NPM_TOKEN` secret，按需配置 `npm` environment 的审批，再手动运行 `Publish npm`，使用 `v0.4.1` tag 作为 ref，例如 `gh workflow run npm-publish.yml --ref v0.4.1`。工作流先在 Windows、macOS、Linux runner 上测试并验证各自当前架构的安装包，全部通过后再构建六个平台并依次发布原生包和启动器。`Build Release` 可通过 tag 或手动触发，不是 npm 发布的前置依赖。
...
```bash
npx -y @fruitbars/xfyun-ai-mcp@0.4.1 --version
```
