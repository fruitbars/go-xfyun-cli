...
转写结果默认以交错的发音人对话稿展示。CLI 使用 `--transcript-format`，MCP 结果查询使用 `transcript_format`，可选 `dialogue`（默认）、`text`、`timeline`、`speaker_grouped`、`srt` 和 `vtt`。底层 `utterances[]` 始终按讯飞 lattice 原始顺序保留发音人、声道、文本和时间轴。

## 本项目的核心范围

默认验收和日常使用只依赖普通录音转写：`result_type=transfer`。翻译、质检、系统词典、热词、候选、多语种分析、回调和其他高级定制都不是必需能力；未显式配置时，客户端不会发送相应参数，也不会因为账号未开通这些权限而失败。核心能力仍包括超限本地自动切片、异步查询、发音人分离、双声道分轨、时间戳和按发音人输出。
...
本地 `input_path` 在提交前由随包媒体探测器读取第一条音频流的声道数：

| 输入/参数 | 实际请求 | 结果 |
| --- | --- | --- |
| 单声道，未设置 `track_mode` | 省略 `trackMode` | 使用讯飞默认混合模式 |
| 双声道，未设置 `track_mode` | 自动发送 `trackMode=2` | 返回左右声道分轨结果 |
| 显式 `track_mode=1` | 发送 `trackMode=1` | 强制混合双声道 |
| 显式 `track_mode=2` | 发送 `trackMode=2` | 强制双声道分轨，输入必须真实双声道 |
| 显式 `role_type` 或语种分析 | 不自动改成 `2` | 保留角色/分析请求，避免与 `trackMode=2` 互斥 |
| `audio_url` 外链 | 不探测 | 仅使用显式 `track_mode` 或服务默认值 |

声道探测失败不会阻止任务提交，工具会回退到省略 `trackMode`。因此单声道普通转写不会额外发送该参数；只有用户显式指定，或本地文件被确认是双声道且没有互斥选项时，才会发送它。
