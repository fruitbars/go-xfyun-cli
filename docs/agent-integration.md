...
这不是全局安装；`npx` 会下载并缓存包，`--version` 只做启动验收。下面的 CLI 快速注册使用 `@latest`；团队和生产环境推荐固定版本（当前为 `@0.8.0`），保证配置可复现。WorkBuddy 连接器仍按其单独的审核版本发布。
...
command = "npx"
args = ["-y", "@fruitbars/xfyun-ai-mcp@0.8.0"]
...
提交 <绝对音频路径> 做录音文件转写；若尚未完成，只查询一次状态并保留任务标识。
使用 `variant="standard"` 提交标准版录音转写；标准版只需保存 `order_id`，默认语言为 `cn`。
把 <HTTP(S) 音频 URL> 作为外链录音转写，并提供带后缀的文件名和准确字节数。
```

应看到六个工具：`xfyun_ocr`、`xfyun_tts`、`xfyun_rtasr`、`xfyun_ifasr_submit`、`xfyun_ifasr_result`、`xfyun_media`。本地文件路径必须对 MCP Server 所在机器可见；IFASR 外链输入由讯飞服务器通过 HTTP(S) 下载。对长 PDF 可先调用 `xfyun_ocr` 的 `dry_run=true` 做无云预检；文件类成功结果使用 `artifacts[]` 传递路径、MIME、大小和 SHA-256。`xfyun_media` 可在转写前查询或转换本地音频的声道、采样率和码率。

IFASR 大模型版普通任务必须保留 `order_id` 和 `signature_random`；标准版只需 `order_id`。自动切片任务必须保留完整 `parts` 并作为 `orders` 查询。状态 `0/3` 可以继续轮询，`4` 才表示完成，`-1` 应检查 `fail_type`。批量文件应在每次提交成功后立即保存任务标识，避免宿主中断后重复上传。详细流程和错误码见 [IFASR 使用指南](ifasr.md)。
