...
- 单个 PDF 文件最大 500 MiB，客户端不再按页数硬拒绝；但选中的页数超过 1000 页时，工具会在第一次 OCR 请求前要求显式确认，因为这将产生至少 1001 次 OCR 请求。MCP 使用 `confirm_large_pdf=true`，CLI 使用 `--confirm-large-pdf`；Agent 应先把页数、预计请求量和耗时告知用户。PDF 会逐页渲染、识别并写入 NDJSON；`pages="1-200"`、`pages="201-400"` 等范围仍可用于超大文件分批或失败后续跑。额度、频率、权限和宿主超时由服务端或调用上下文处理。
