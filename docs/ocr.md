...
- 单个 PDF 文件最大 500 MiB。PDF 总页数可以超过 200 页，但每次调用最多选择 200 页；例如一个 600 页 PDF 可分别使用 `pages="1-200"`、`pages="201-400"`、`pages="401-600"` 处理。这个 200 页阈值是客户端为避免误触发超长任务、额度消耗和 MCP 宿主超时设置的单次调用保护，不是讯飞接口或 PDF 文件格式的总页数限制；超过 200 页时省略 `pages` 会提示按范围选择。
