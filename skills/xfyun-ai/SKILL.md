---
name: xfyun-ai
description: Use XFYun AI tools for document-image OCR, speech synthesis, real-time audio transcription, asynchronous recording transcription, or local audio preparation. Apply when the user requests XFYun AI processing or when these configured tools are the intended provider.
---
...
- When the user asks to mark, visualize, or inspect OCR layout element types on the image, set `annotate=true`. Use `annotation_types` for requested types or `all`; single-page calls return an inline PNG and `annotation_path`, while multi-page calls return `annotation_paths`.
